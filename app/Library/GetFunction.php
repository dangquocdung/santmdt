...
class GetFunction
{
  ...
  {
    $new_ary = array();
    $method  = array('bacs' => Lang::get('admin.direct_bank_transfer'), 'cod' => Lang::get('admin.cash_on_delivery'), 'paypal' => Lang::get('admin.paypal'), 'stripe' => Lang::get('admin.stripe'));
  ...
  public static function vendor_details_by_product_id($product_id){
    $vendor_final = array();
    $get_author_id  = Post::where(['id' => $product_id, 'post_type' => 'product'])->first();
   
    if(!empty($get_author_id)){
      $get_role_id = RoleUser::where(['user_id' => $get_author_id->post_author_id])->first();
      ...
      if(!empty($get_role_id)){
        ...
        if(!empty($get_vendor_details) && $get_vendor_details->slug == 'vendor'){
          $vendor_details = get_user_details( $get_author_id->post_author_id );
          $get_account_details = get_user_account_details_by_user_id( $get_author_id->post_author_id );
  ...
  public static function vendor_id_by_product_id($product_id){
    ...
    if(!empty($get_author_id)){
      ...
      if(!empty($get_role_id)){
        ...
        if(!empty($get_vendor_details) && $get_vendor_details->slug == 'vendor'){
          $vendor_id = $get_author_id->post_author_id;
        }
      }
    }
  ...
  public static function author_total_products($author_id){
    $total_products = 0;
    $get_author_id  = Post::where(['post_author_id' => $author_id, 'post_type' => 'product'])->get()->toArray();
    
    if(count($get_author_id) > 0){
  ...
  public static function author_id_by_product_id($product_id){
    $author_id = null;
    $get_post = Post::where(['id' => $product_id, 'post_type' => 'product'])->first();
    
    if(!empty($get_post)){
      $author_id = $get_post->post_author_id; 
    }
