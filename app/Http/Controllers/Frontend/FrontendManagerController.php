...
use shopist\Http\Controllers\Admin\InstallationController;
...
class FrontendManagerController extends Controller
{
  ...
  public function goToDifferentPages($params = null, $params2 = null)
  {
    ...
    if(!$install->isAdminRoleDataExist()){
    ...
    if($params && (Request::is('product/details/*') || Request::is('product/customize/*'))){
      ...
      if(count($get_post) > 0){
        ...
        $this->frontendData['vendor_reviews_rating_details']  =   get_comments_rating_details( get_vendor_id_by_product_id($product_id), 'vendor' );
    ...
    if($params && Request::is('product/categories/*')){
      ...
      if(count($this->frontendData['product_by_cat_id']) > 0){
        ...
        elseif(isset($_GET['view']) && $_GET['view'] == 'grid'){
          ...
          $this->frontendData['product_by_cat_id']['selected_view'] = 'grid';
			...
			if($params && Request::is('store/details/cat/products/*')){
      $sort = null;
      $price_min = null;
      $price_max = null;
      $selected_colors = null;
      ...
      if(count($get_cat_product_and_breadcrumb) > 0){
        $this->frontendData['vendor_products'] = $get_cat_product_and_breadcrumb;
      }
      else{
        return view('errors.no_data');
      }
      ...
      if(count($this->frontendData['vendor_products']) > 0){
        $this->frontendData['vendor_products']['action_url'] = Request::url();
        ...
        if(count($currentQuery) > 0){
          ...
          if(count($currentQuery) > 0){
            $currentQuery['view'] = 'list';
            $this->frontendData['vendor_products']['action_url_list_view'] = Request::url(). '?' . http_build_query($currentQuery);
            $currentQuery['view'] = 'grid';
            $this->frontendData['vendor_products']['action_url_grid_view'] = Request::url(). '?' . http_build_query($currentQuery);
          }
          else{
            $this->frontendData['vendor_products']['action_url_list_view'] = Request::url(). '?view=list';
            $this->frontendData['vendor_products']['action_url_grid_view'] = Request::url(). '?view=grid';
          }
        }
        else{
          $this->frontendData['vendor_products']['action_url_list_view'] = Request::url(). '?view=list';
        ...
        if(isset($_GET['view']) && $_GET['view'] == 'list'){
          $this->frontendData['vendor_products']['selected_view'] = 'list'; 
        }
        elseif(isset($_GET['view']) && $_GET['view'] == 'grid'){
          $this->frontendData['vendor_products']['selected_view'] = 'grid'; 
        }
        else{
          $this->frontendData['vendor_products']['selected_view'] = 'grid';
        }
    ...
    if(Request::is('cart') || Request::is('checkout')){
      ...
      if(!empty($vendor_details)){
        $this->frontendData['shipping_data'] = $this->classCommonFunction->objToArray($vendor_details->shipping_method, true);
      }
      else{
        $this->frontendData['shipping_data'] = $this->option->getShippingMethodData();
      }
      
      $this->frontendData['payment_method_data'] = $this->option->getPaymentMethodData();
    ...
    if(Request::is('stores')){
      $this->frontendData['vendors_list'] = $this->vendors->getAllVendors( false, null, 1 );
    }
    
    if( ($params || $params2) && ( Request::is('store/details/home/*') || Request::is('store/details/products/*') || Request::is('store/details/reviews/*') || Request::is('store/details/cat/products/*'))){
      ...
      $this->frontendData['vendor_settings'] = null;
      $this->frontendData['vendor_selected_cats_id'] = array();
      ...
      if(!empty($get_user)){
        ...
        $this->frontendData['vendor_package_details'] = get_package_details_by_vendor_id($get_user->id);
        $this->frontendData['vendor_settings'] = null;
        ...
        $this->frontendData['vendor_info'] = $get_user;
        $this->frontendData['vendor_settings'] = $get_user_details;
								
        
        if(Request::is('store/details/home/*') || Request::is('store/details/products/*') || Request::is('store/details/cat/products/*')){
          $this->frontendData['vendor_home_page_cats'] = array();
            ...
            if(count($vendor_home_cats) > 0){
              foreach($vendor_home_cats as $cat){
                ...
                if(Request::is('store/details/products/*') || Request::is('store/details/cat/products/*')){
                    array_push($this->frontendData['vendor_selected_cats_id'], $get_id);
                }
                array_push($this->frontendData['vendor_home_page_cats'], array('parent_cat' => array_shift($parent_cat), 'child_cat' => $this->product->get_categories($get_id, 'product_cat')));
              }
            }
          }
          
          $this->frontendData['vendor_advanced_items'] = $this->product->getVendorAdvancedProducts( $get_user->id );
          $this->frontendData['vendor_reviews_rating_details']  =   get_comments_rating_details( $get_user->id, 'vendor' );
        }
        ...
        if(Request::is('store/details/products/*')){
          ...
          if(count($get_product) > 0){
              $this->frontendData['vendor_products'] = $get_product;
        ...
        if(Request::is('store/details/reviews/*')){
          $this->frontendData['comments_details']         =  get_comments_data_by_object_id( $get_user->id, 'vendor' );
          $this->frontendData['comments_rating_details']  =  get_comments_rating_details( $get_user->id, 'vendor' );
          $this->frontendData['vendor_reviews_rating_details']  =   get_comments_rating_details( $get_user->id, 'vendor' );
          $this->frontendData['user_name']                =  $user;
        }
        ...
        $this->frontendData['store_seo_meta_keywords'] = null;
        $this->frontendData['store_seo_meta_description'] = null;
        
        if(isset($get_global_seo['meta_tag']['meta_keywords']) && !empty($store_seo->meta_keywords)){
          $this->frontendData['store_seo_meta_keywords'] = trim($get_global_seo['meta_tag']['meta_keywords'].', '.$store_seo->meta_keywords, ',');
        }
        elseif(isset($get_global_seo['meta_tag']['meta_keywords']) && empty($store_seo->meta_keywords)){
          $this->frontendData['store_seo_meta_keywords'] = $get_global_seo['meta_tag']['meta_keywords'];
        }
        else{
          $this->frontendData['store_seo_meta_keywords'] = $store_seo->meta_keywords;
        }
        
        if(!empty($store_seo->meta_decription)){
          $this->frontendData['store_seo_meta_description'] = $store_seo->meta_decription;
        }
      }
      else{
        return view('errors.no_data');
