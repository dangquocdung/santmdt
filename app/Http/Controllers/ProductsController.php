...
class ProductsController extends Controller
{
  ...
  public function saveProduct($params = null){
    if( Request::isMethod('post') && Session::token() == Input::get('_token') ){
      ...
      if(is_vendor_login() && Session::has('shopist_admin_user_id') && Input::get('hf_post_type') == 'add_post'){
        ...
        $get_total_product = Post::where(['post_author_id' => Session::get('shopist_admin_user_id'), 'post_type' => 'product'])->first();
        ...
        if( ( $skuCheck && $sku_id == $product_id ) ||  $skuCheck == null){
          ...
          elseif (Input::get('hf_post_type') == 'update_post'){
            ...
            if( Post::where('id', $product_id)->update($data)){
              ...
              PostExtra::where(['post_id' => $product_id, 'key_name' => '_selected_vendor'])->update($selected_vendor);
  ...
  */
   ...
   * @return array
   */
  public function getPosts($pagination = false, $search_val = null, $status_flag = -1, $author_id){
    $data_array = array();
    ...
    if((is_vendor_login() && Session::has('shopist_admin_user_id')) || (!empty($author_id) && $author_id > 0 && $author_id != 'all')){
      ...
      if($status_flag == -1){
        $where = ['post_author_id' => $post_author_id, 'post_type' => 'product'];
      }
      else{
          $where = ['post_author_id' => $post_author_id, 'post_type' => 'product', 'post_status' => $status_flag];
      }
    }
    else{
      if($status_flag == -1){
        ...
        $where = ['post_type' => 'product', 'post_status' => $status_flag];
      }
    }
    ...
    if(count($get_posts_for_product)>0){
      foreach($get_posts_for_product as $rows){
        ...
        foreach($get_post_meta as $val){
          if($val->key_name == '_product_related_images_url'){
            $rows[$val->key_name] = json_decode($val->key_value);
            $rows['product_related_img_json'] = $val->key_value;
          }
          elseif($val->key_name == '_product_custom_designer_panel_size' || $val->key_name == '_product_video_feature_panel_size' || $val->key_name == '_product_selected_categories' || $val->key_name == '_product_selected_tags'){
            $rows[$val->key_name] = unserialize($val->key_value);  
          }
          elseif($val->key_name == '_product_custom_designer_data'){
            $rows[$val->key_name] = json_decode($val->key_value);
            $rows['product_custom_designer_json'] = $val->key_value;
          }
          else{
            $rows[$val->key_name] = $val->key_value;  
          }
        }
      }
    }
    ...
    if(count($filter_arr) > 0){
      foreach($filter_arr as $term_filter){
        $get_posts_for_product  = DB::table('posts')
																																		->where(['post_author_id' => $user_id, 'posts.post_type' => 'product', 'posts.post_status' => 1]);
  ...
  */
  ...
  public function getProductCommentsList(){
    ...
    if(is_vendor_login() && Session::has('shopist_admin_user_id')){
      $where = ['posts.post_author_id' => Session::get('shopist_admin_user_id'), 'comments.target' => 'product'];
    }
    else{
  ...
  public function getVendorAdvancedProducts( $vendor_id ){
    ...
    if(count($get_recommended_items)>0){
      foreach($get_recommended_items as $items1){
        ...
        if(isset($get_post_for_recommended['post_type']) && $get_post_for_recommended['post_type'] == 'product' && $get_post_for_recommended['author_id'] == $vendor_id){
          if(count($get_post_for_recommended)>0){
            array_push($recommended_arr, $get_post_for_recommended);
          }
        }
    ...
    if(count($get_features_items)>0){
      foreach($get_features_items as $items2){
        ...
        if(isset($get_post_for_features['post_type']) && $get_post_for_features['post_type'] == 'product' && $get_post_for_features['author_id'] == $vendor_id){
          if(count($get_post_for_features)>0){
            array_push($features_arr, $get_post_for_features);
          }
        }
    ...
    if(count($get_latest_items)>0){
      foreach($get_latest_items as $items3){
        ...
        if(isset($get_post_for_latest['post_type']) && $get_post_for_latest['post_type'] == 'product' && $get_post_for_latest['author_id'] == $vendor_id){
          if(count($get_post_for_latest)>0){
            array_push($latest_arr, $get_post_for_latest);
          }
        }
    ...
    if(count($get_best_sales) > 0){
      foreach($get_best_sales as $items4){
        ...
        if(isset($get_post_for_best_sales['post_type']) && $get_post_for_best_sales['post_type'] == 'product' && $get_post_for_best_sales['author_id'] == $vendor_id){
          if(count($get_post_for_best_sales)>0){
            array_push($best_sales_arr, $get_post_for_best_sales);
          }
        }
            ...
            foreach($parse as $items6){
              if(isset($items6['id'])){
                if(!$this->classCommonFunction->is_item_already_exists_in_array($items6['id'], $todays_deal_arr)){
                  ...
                  if(isset($get_post_for_todays_deal['post_type']) && $get_post_for_todays_deal['post_type'] == 'product' && $get_post_for_todays_deal['author_id'] == $vendor_id){
                    if(count($get_post_for_todays_deal) > 0){
                      array_push($todays_deal_arr, $get_post_for_todays_deal);
                    }
                  }
  ...
  public function getFilterProductsDataWithPagination( $filter = array() ){
    ...
    if(count($filter_arr) > 0){
      ...
      if(isset($filter['srch_term'])){
        $get_posts_for_product->where('posts.post_title', 'like', '%'. $filter['srch_term'] .'%');
      }
      ...
      if(isset($filter['sort']) && $filter['sort'] == 'alpaz'){
        $get_posts_for_product->orderBy('posts.post_title', 'ASC');
      }
      elseif (isset($filter['sort']) && $filter['sort'] == 'alpza') {
        $get_posts_for_product->orderBy('posts.post_title', 'DESC');
      }
      elseif (isset($filter['sort']) && $filter['sort'] == 'low-high') {
        ...
        $get_posts_for_product->orderBy('post_extras.key_value', 'ASC');
      }
      elseif (isset($filter['sort']) && $filter['sort'] == 'high-low') {
        ...
        $get_posts_for_product->orderBy('post_extras.key_value', 'DESC');
      }
      elseif (isset($filter['sort']) && $filter['sort'] == 'old-new') {
        $get_posts_for_product->orderBy('posts.created_at', 'ASC');
      }
      elseif (isset($filter['sort']) && $filter['sort'] == 'new-old') {
        $get_posts_for_product->orderBy('posts.created_at', 'DESC');
      }
  ...
  public function manageExportProducts(){
    ...
    if(is_vendor_login() && Session::has('shopist_admin_user_id')){
      $get_products = Post :: where(['post_author_id' => Session::get('shopist_admin_user_id'), 'post_type' => 'product'])->orderBy('id', 'DESC')->get()->toArray();
    }
    else{
